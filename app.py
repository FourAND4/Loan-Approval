...
dan scaling sudah dibungkus di dalam `loan_pipeline.pkl` -- app ini TIDAK
...
try:
    ...
    st.error(
        "File `loan_pipeline.pkl` tidak ditemukan di folder yang sama dengan `app.py`. "
...
# Tenor: satuan HARI utk tampilan, TAPI nilai bulan yg dikirim ke model
# WAJIB tetap 12-480 bulan (rentang asli data training) -- kalau tidak,
# EMI & LoanIncomeRatio jadi tidak wajar (dibagi tenor sangat kecil) dan
# model selalu menolak apa pun gaji/pinjamannya. Jadi hari = bulan * 30.
TENOR_OPTIONS_BULAN = [480, 360, 300, 240, 180, 120, 84, 60, 36, 12]
TENOR_OPTIONS = [b * 30 for b in TENOR_OPTIONS_BULAN]  # dalam hari (360-14.400)
...
c2.metric("✅ Akurasi", f"{info.get('test_accuracy', 0) * 100:.1f}%")
...
with col_r:
    ...
    tenor_pinjaman_hari = st.selectbox(
        "Tenor Pinjaman (hari)", TENOR_OPTIONS,
        index=TENOR_OPTIONS.index(360 * 30),
        format_func=lambda x: f"{x:,} hari  (~{x // 30} bulan)".replace(",", "."),
    )
    tenor_pinjaman = tenor_pinjaman_hari / 30  # konversi ke BULAN -- satuan yang dipakai model
...
if predict_btn:
    ...
    fact_rows = [
        ...
        ("Tenor", f"{tenor_pinjaman_hari:,} hari (~{int(tenor_pinjaman)} bulan)".replace(",", "."),
    ...
    if total_income > 0 and emi_bulanan > total_income * 0.5:
        recs.append((P["orange"], f"⚠️ Cicilan Terlalu Berat ({emi_bulanan / total_income * 100:.0f}% pendapatan)",
                     f"Pertimbangkan tenor lebih panjang (10.800–14.400 hari / 360–480 bulan) untuk menurunkan cicilan bulanan."))
...
with st.expander("ℹ️ Tentang model ini"):
    ...
    st.write(f"**Akurasi (test set)**: {info.get('test_accuracy', '-')}")
    st.write(f"**F1-score (test set)**: {info.get('test_f1', '-')}")
    st.write(f"**ROC-AUC (test set)**: {info.get('test_roc_auc', '-')}")
    st.caption(
